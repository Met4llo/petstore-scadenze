...
function showPage(pageId) {
  ...
  if (pageId === 'scanner') {
    if (typeof unlockScanFeedback === 'function') unlockScanFeedback();
    startScanner();
  }
...
async function pickBackCameraId() {
  ...
  try {
    ...
    const backs = cams.filter(c => /back|rear|environment|dietro|indietro|trasera/i.test(c.label || ''));
    const pool = backs.length ? backs : cams;
    const wide = pool.find(c => {
      const lab = c.label || '';
      if (/ultra|tele|macro|truedepth|front|frontale/i.test(lab)) return false;
      return /wide|grandangolo|posterior/i.test(lab) || /back|rear|dietro/i.test(lab);
    });
    return (wide || pool[0] || cams[0]).id;
  } catch (e) {
    return null;
  }
}

function showScanFocusReticle(clientX, clientY) {
  const box = document.getElementById('reader');
  if (!box) return;
  const rect = box.getBoundingClientRect();
  let el = box.querySelector('.scan-focus-reticle');
  if (!el) {
    el = document.createElement('div');
    el.className = 'scan-focus-reticle';
    el.setAttribute('aria-hidden', 'true');
    box.appendChild(el);
  }
  ...
  clearTimeout(showScanFocusReticle._t);
  showScanFocusReticle._t = setTimeout(() => { if (el) el.classList.remove('is-on'); }, 750);
}

async function applyTapFocus(nx, ny) {
  ...
  const tries = [];
  const poi = [{ x: Math.min(1, Math.max(0, nx)), y: Math.min(1, Math.max(0, ny)) }];
  if (cap.pointsOfInterest) {
    tries.push({ advanced: [{ pointsOfInterest: poi }] });
    if (cap.focusMode && cap.focusMode.indexOf('manual') >= 0) {
      tries.push({ advanced: [{ focusMode: 'manual', pointsOfInterest: poi }] });
    }
  }
  if (cap.focusMode) {
    if (cap.focusMode.indexOf('single-shot') >= 0) tries.push({ advanced: [{ focusMode: 'single-shot' }] });
    if (cap.focusMode.indexOf('continuous') >= 0) tries.push({ advanced: [{ focusMode: 'continuous' }] });
  }
  ...
  if (!isAppleMobile() && cap.zoom) {
    ...
    tries.push({ advanced: [{ zoom: bumped }] });
  }
  for (let i = 0; i < tries.length; i++) {
    try {
      await track.applyConstraints(tries[i]);
      if (tries[i].advanced && tries[i].advanced[0] && tries[i].advanced[0].zoom != null && cap.zoom) {
        const z0 = (track.getSettings() || {}).zoom;
        setTimeout(() => {
          try { track.applyConstraints({ advanced: [{ zoom: z0 || cap.zoom.min || 1 }] }); } catch (e) {}
        }, 180);
      }
      return true;
    } catch (e) {}
...
function handleScanTapFocus(ev) {
  ...
  showScanFocusReticle(ev.clientX, ev.clientY);
  applyTapFocus(nx, ny);
}

function bindScanTapFocus() {
  const box = document.getElementById('reader');
  if (!box || box.dataset.focusBound) return;
...
async function startScanner() {
  ...
  if (cameraId) {
    ...
    attempts.push({ cam: cameraId, cfg: { fps: 14, qrbox: qrbox, disableFlip: true } });
  }
  attempts.push({
    cam: { facingMode: 'environment' },
    cfg: {
...
function provaGuardLocked(action) {
  ...
  showToast('Settimana chiusa: solo consultazione' + (canEditTurniProva() ? '. Puoi sbloccare per una correzione.' : '.'), 'warn');
  return true;
}

function unlockProvaWeek() {
  if (!canEditTurniProva()) {
    showToast('Solo Fuschi e Santoemma possono sbloccare una settimana chiusa', 'warn');
    return;
  }
...
function applyProvaLockUi() {
  ...
  if (unlock) {
    unlock.classList.toggle('hidden', !(provaSundayPassed() && provaIsLocked() && canEdit));
  }
...
function renderProvaCheck() {
  ...
  if (provaSundayPassed()) {
    ...
    el.innerHTML = provaIsLocked()
      ? 'Settimana chiusa — solo consultazione.' + (canEditTurniProva() ? ' Puoi sbloccare per una correzione.' : '')
...
const MONTE_SENTINEL = '1970-01-01';
let monteMovs = [];
let monteAuto = [];
let monteTableMissing = false;
let monteSaving = false;
...
function normalizeMonteMov(m) {
  if (!m || typeof m !== 'object') return null;
  const ore = Number(m.ore);
  if (!m.operatore || !Number.isFinite(ore) || !ore) return null;
  return {
    id: String(m.id || newMonteId()),
...
function isMonteSentinelRow(row) {
  if (!row) return false;
  if (String(row.settimana_inizio || '') === MONTE_SENTINEL) return true;
  const celle = row.celle;
  return !!(celle && celle._tipo === 'monte_ore');
}

async function persistMonteCloud(list) {
  const clean = (list || []).map(normalizeMonteMov).filter(Boolean);
  writeLocalMonte(clean);
  if (!supabase) return { ok: false, offline: true };
  const { error } = await supabase.from('turni_prova').upsert({
    settimana_inizio: MONTE_SENTINEL,
    celle: { _tipo: 'monte_ore', _monte: clean },
    updated_by: currentOperator || '',
    updated_at: new Date().toISOString()
  }, { onConflict: 'settimana_inizio' });
...
async function loadMonteOre() {
  monteAuto = [];
  const local = readLocalMonte().map(normalizeMonteMov).filter(Boolean);
  monteMovs = local;
  if (supabase) {
    try {
      const weeks = await supabase.from('turni_prova').select('settimana_inizio,celle');
      if (!weeks.error && weeks.data) {
        monteAuto = computeMonteAuto(weeks.data);
        const sent = weeks.data.find(isMonteSentinelRow);
        const raw = sent && sent.celle && Array.isArray(sent.celle._monte) ? sent.celle._monte : [];
        const cloud = raw.map(normalizeMonteMov).filter(Boolean);
        const byId = new Map();
        cloud.forEach(m => byId.set(String(m.id), m));
        const extras = [];
        local.forEach(m => {
          if (!byId.has(String(m.id))) extras.push(m);
        });
        monteMovs = cloud.concat(extras);
        writeLocalMonte(monteMovs);
        if (extras.length) await persistMonteCloud(monteMovs);
      }
    } catch (e) {}
  }
  renderMonteOre();
...
async function addMonteRettifica() {
  ...
  const row = normalizeMonteMov({
    id: monteEditId || newMonteId(),
    operatore: op,
    data: data,
    ore: ore,
    motivo: motivo,
  ...
  if (monteEditId) {
    const prev = monteMovs.find(x => String(x.id) === String(monteEditId));
    if (prev && prev.created_by) row.created_by = prev.created_by;
    row.id = String(monteEditId);
    monteMovs = monteMovs.map(x => String(x.id) === String(monteEditId) ? row : x);
  } else {
    monteMovs = [row].concat(monteMovs);
  }
  const wasEdit = !!monteEditId;
  monteSaving = true;
  const btn = document.getElementById('btn-monte-add');
  if (btn) btn.disabled = true;
  const res = await persistMonteCloud(monteMovs);
  monteSaving = false;
  if (btn) btn.disabled = false;
  if (!res.ok && !res.offline) {
    showToast('Errore salvataggio: ' + ((res.error && res.error.message) || 'riprova'), 'error');
    await loadMonteOre();
    return;
  }
  monteClearForm();
...
async function deleteMonteRettifica(id) {
  ...
  const res = await persistMonteCloud(monteMovs);
  if (!res.ok && !res.offline) {
    showToast('Errore: ' + ((res.error && res.error.message) || 'riprova'), 'error');
    await loadMonteOre();
    return;
  }
...
/** Solo Fuschi e Santoemma possono modificare Turni 2.0; gli altri solo visualizzano. */
function canEditTurniProva() {
  return currentOperator === 'Santoemma' || currentOperator === 'Fuschi';
}
